...
export function Diya({ className = '', delay = 0 }) {
    return (
        <motion.div className={`relative w-16 h-16 ${className}`} initial={{
            scale: 0,
            opacity: 0
        }} whileInView={{
            scale: 1,
            ...
            {/* Glow effect */}
            <motion.div className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-8 rounded-full bg-yellow-400 blur-xl" animate={{
                opacity: [0.2, 0.5, 0.2]
            }} transition={{
                repeat: Infinity,
                duration: 2
