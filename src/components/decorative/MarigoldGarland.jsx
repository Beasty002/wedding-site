...
export function MarigoldGarland({ delay = 0, className = '' }) {
    return (
        <motion.div className={`absolute top-0 z-10 ${className}`} initial={{
            y: -100,
            opacity: 0
        }} animate={{
            y: 0,
            ...
            <motion.svg width="100" height="300" viewBox="0 0 100 300" fill="none" xmlns="http://www.w3.org/2000/svg" style={{
                originY: 0
            }} animate={{
                rotate: [-2, 2, -2],
                scaleY: [1, 1.02, 1]
                ...
                {[...Array(6)].map((_, i) => (
                    <g key={i} transform={`translate(50, ${40 + i * 45})`}>
                        <motion.circle r="20" fill="#FF9933" initial={{
                            scale: 0
                        }} animate={{
                            scale: 1
                        }} transition={{
                        ...
                        {/* Petal details */}
                        {[...Array(8)].map((_, j) => (
                            <path key={j} d={`M0 0 L${20 * Math.cos(j * Math.PI / 4)} ${20 * Math.sin(j * Math.PI / 4)}`} stroke="#FF6B00" strokeWidth="1" opacity="0.5" />
                        ))}
                    </g>
                ))}
